...
/* 패킷 요약 정보 출력 함수 */
void print_packet_summary(int packet_no, analysis_data_t *data) {
    struct pcap_pkthdr *header = &data->headers[packet_no - 1];
    const u_char *packet = data->packets[packet_no - 1];

    /* 패킷 캡처 시간 계산 */
    double elapsed_time = header->ts.tv_sec + header->ts.tv_usec / 1000000.0;
  ...
  /* 사용자 입력 처리 */
    ...
    while (continue_flag && current_packet <= analysis_data.total_packets) {
        ...
        if (strcmp(input, "0") == 0) {
            ...
            for (int i = current_packet; i <= analysis_data.total_packets; i++) {
                print_packet_summary(i, &analysis_data);
            }
            continue_flag = 0;
            break;
        }
